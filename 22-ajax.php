<?php
	...
	switch(strtolower($ACTION))
	{
		...
		case 'status':
			...
			$since = parse_since($GLOBALS['SINCE']);
			if (false !== $bounds)
			{
	...
	function parse_since($since)
	{
		...
		if (empty($since))
			return false;
		$date = date_create_from_format("YmdHis", $since);
		$parsed = date_format($date, 'Y-m-d H:i:s');
		return $parsed;
	}
